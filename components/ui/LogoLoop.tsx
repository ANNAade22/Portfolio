...
const useAnimationLoop = (
  ...
  useEffect(() => {
    ...
    // Initialize transform when dimensions are available
    const initialSeqSize = isVertical ? seqHeight : seqWidth;
    if (initialSeqSize > 0) {
      offsetRef.current = ((offsetRef.current % initialSeqSize) + initialSeqSize) % initialSeqSize;
    ...
    const animate = (timestamp: number) => {
      ...
      if (currentSeqSize > 0) {
        // Normalize offset if sequence size changed
